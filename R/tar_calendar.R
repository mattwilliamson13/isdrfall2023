...
build_schedule_for_page <- function(schedule_file) {
  schedule <- read_csv(schedule_file, show_col_types = FALSE) %>%
    ...
    mutate(date_range = ifelse(!is.na(end_date),
                               glue('{format(date, "%B %e")}'), glue('{format(date, "%B %e")}'))) %>% 
    mutate(col_date = case_when(
      !is.na(content) ~ ifelse(is.na(end_date), 
                               glue('<span class="content-date">{format(date, "%B %e")}</span><br><span class="content-session">({session})</span>'), 
                               glue('<span class="content-date">{format(date, "%B %e")}</span><br><span class="content-session">({session})</span>')),
      is.na(content) & !is.na(date) ~ glue('<span class="content-date">{date_range}</span>'),
